...
创建信息统一使用格式为`分支类型前缀/功能`。

常用类型：`feature/(开发新功能、新需求)`、`bugfix/(修复一般的 Bug（通常从 develop 分支拉取）)`、`hotfix/(紧急修复线上生产环境的 Bug（直接从 main 拉取）)`、`refactor/(代码重构（没有新功能，也没有修 Bug）)`、`docs/(只修改了文档、ReadMe 等)`、`test/(增加或修改单元测试、自动化测试代码)`
